...
import argparse
import logging
# dill 是增强版的pickle，可以序列化更多的python对象
...
def _download_file(download_dir, url):
    ...
    if file_exist(download_dir, filename):
        ...
        with TqdmUpTo(unit='B', unit_scale=True, miniters=1, desc=filename) as t:
             # python 官方的urllib.request 用于从指定的url下载文件，reporthook是一个回调函数，反应实时下载进度
...
def main_wo_bpe():
    ...
    # argparse 是python的一个命令行解析包，可以用来方便地设定和读取命令行参数
    parser = argparse.ArgumentParser()
    # '-'，命名习惯，表示是参数
    parser.add_argument('-lang_src', required=True, choices=spacy_support_langs)
    parser.add_argument('-lang_trg', required=True, choices=spacy_support_langs)
    # pkl文件是序列化和反序列化操作的文件格式，可以保存各种数据类型，用python的pickle官方模块
    parser.add_argument('-save_data', required=True) 
    ...
    # share the vocabulary between source and target
    parser.add_argument('-share_vocab', action='store_true') 
    #parser.add_argument('-ratio', '--train_valid_test_ratio', type=int, nargs=3, metavar=(8,1,1))
    #parser.add_argument('-vocab', default=None)

    opt = parser.parse_args()
    # assert断言在开发和测试中有用，但是不能代替错误处理机制，因为它在生产环境中可能会被关闭
    assert not any([opt.data_src, opt.data_trg]), 'Custom data input is not support now.' # 向用户说明，现在不支持自定义数据输入
    ...
    # 定义新的tokenize函数，用于分词
    # spacy的分词器返回的是一个生成器，所以要用list()转换成列表
    # spacy会生成Doc对象，.text是为了返回原始文本
    ...
    # https://github.com/pytorch/text/issues/664 提供了为什么不再使用torchtext.data.Field的原因
    # torchtext == 0.6.0 降级了使用！
